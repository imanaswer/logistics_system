...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    def get_display_party_name(self, obj):
        """
        Logic: 
        1. Use manually typed party_name if exists.
        2. Else, use the name from the linked client object.
        3. Else, use the name from the linked job's client.
        4. Fallback to "General Transaction".
        """
        if obj.party_name:
    ...
    def validate(self, data):
        ...
        # If both are provided, ensure they don't contradict each other
        if job and client and job.client_id != client.id:
            raise serializers.ValidationError(
                "Transaction client must match the job's client."
            )
        return data
