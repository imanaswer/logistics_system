...
interface Job {
  ...
  invoice_no?: string; // Added for unique ID visibility
  vat_number?: string; // Added for VAT support
...
export default function EditInvoicePage() {
  ...
  const [vatNo, setVatNo] = useState(""); // State for VAT editing
    ...
    const load = async () => {
      try {
        const [jobRes, chargeRes, itemRes] = await Promise.all([
          ...
          axios.get(`${API_BASE}/invoice-items/`, authConfig),
        ]);
        const jobItems = itemRes.data.filter((i: any) => i.job === Number(jobId));
        setJob(jobRes.data);
        setBillNo(jobRes.data.transport_document_no ?? "");
        setVatNo(jobRes.data.vat_number ?? ""); // Loading existing VAT
        setChargeTypes(chargeRes.data);
        setRows(jobItems.length ? jobItems.map((i: any) => ({
  ...
  const saveInvoice = async () => {
    ...
    setIsSaving(true);
    try {
      // Patch includes VAT number and Doc number
      await axios.patch(`${API_BASE}/jobs/${jobId}/`, { 
        transport_document_no: billNo,
        vat_number: vatNo 
      }, authConfig);
      
      await Promise.all(originalItemIds.map(id => axios.delete(`${API_BASE}/invoice-items/${id}/`, authConfig)));
      await Promise.all(
        rows.filter(r => r.charge_type && Number(r.amount) > 0).map(r =>
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-slate-50 font-black tracking-tighter text-slate-400">LOADING CORE...</div>;
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <button onClick={() => router.back()} className="p-2.5 hover:bg-slate-100 rounded-xl transition-all text-slate-400 hover:text-slate-900">
              <IconBack />
            </button>
            ...
            <div>
              ...
              <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">
                Invoice No: {job?.invoice_no} {/* Unique ID Displayed */}
              </p>
            </div>
          </div>
          <button 
            onClick={saveInvoice} 
            disabled={isSaving}
            className="bg-black hover:bg-blue-700 disabled:bg-slate-300 text-white px-8 py-3 rounded-2xl font-black text-xs uppercase tracking-[0.15em] transition-all shadow-xl shadow-blue-500/10 active:scale-95 flex items-center gap-3"
          >
        ...
        <div className="col-span-12 lg:col-span-8 space-y-6">
          <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center">
              <h2 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em]">Statement of Charges</h2>
              <button onClick={() => setRows([...rows, { ...EMPTY_ROW }])} className="text-blue-600 hover:text-black transition flex items-center gap-1 text-xs font-black uppercase">
                <IconPlus /> Add Row
              </button>
            </div>
              ...
              {rows.map((row, i) => (
                <div key={i} className="group grid grid-cols-12 gap-4 items-end p-5 rounded-2xl bg-slate-50/50 border-2 border-transparent hover:border-blue-100 hover:bg-white transition-all">
                  <div className="col-span-12 md:col-span-4">
                    ...
                    <select
                      className="w-full bg-white border-slate-200 rounded-xl text-sm font-bold h-12 px-4 focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                      value={row.charge_type}
                      onChange={(e) => updateRow(i, "charge_type", e.target.value)}
                    >
                  ...
                  <div className="col-span-10 md:col-span-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block text-right">Amount</label>
                    <input
                      type="number"
                      className="w-full bg-white border-slate-200 rounded-xl text-sm font-black h-12 px-4 text-right focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                      value={row.amount}
        ...
        <div className="col-span-12 lg:col-span-4 space-y-6">
          ...
          <div className="bg-blue-600 rounded-[2rem] p-8 text-white shadow-2xl shadow-blue-200 relative overflow-hidden">
            <div className="relative z-10">
              ...
              <p className="text-xl font-black leading-tight mb-6">{job?.client?.name || job?.client_details?.name || "Customer"}</p>
              
              <div className="grid grid-cols-1 gap-4 pt-4 border-t border-blue-500">
                <div>
                  <label className="text-[10px] font-black text-blue-200 uppercase">VAT Number</label>
                  <input 
                    value={vatNo}
                    onChange={(e) => setVatNo(e.target.value)}
                    placeholder="VAT/TRN..."
                    className="w-full bg-blue-700/50 border-none rounded-xl px-4 py-3 text-sm font-black placeholder:text-blue-400 focus:ring-2 focus:ring-white outline-none"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black text-blue-200 uppercase">Transport Doc No.</label>
                  <input 
                    value={billNo}
          ...
          <div className="bg-white rounded-[2rem] border border-slate-200 p-8 shadow-sm">
            <div className="space-y-4">
              <div className="flex justify-between items-center text-xs font-bold uppercase tracking-widest text-slate-400">
                <span>Subtotal</span>
                <span className="text-slate-900">{totals.subtotal.toFixed(3)}</span>
              </div>
              <div className="flex justify-between items-center text-xs font-bold uppercase tracking-widest text-slate-400">
                <span>VAT (5%)</span>
              ...
              <div className="pt-6 mt-2 border-t border-slate-100 flex justify-between items-end">
                <span className="text-xs font-black uppercase text-slate-900">Total OMR</span>
                <span className="text-4xl font-black tracking-tighter text-slate-900 tabular-nums">
                  {totals.grand.toFixed(3)}
                </span>
              </div>
            </div>
          </div>
